...
def main():
	...
	predictions.to_csv(basepath.joinpath("output/linkscores.csv"), index=False)

def load_data():
	data = pd.read_csv(basepath.joinpath("downloads/link_scores.csv"))
...
def evaluate(target, preds):
	...
	print(f"Link score accuracy:  {acc:.4f}")
	print(f"Link score precision: {precision:.4f}")
	print(f"Link score recall:    {recall:.4f}")
